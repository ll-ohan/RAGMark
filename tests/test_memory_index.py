...
class TestMemoryIndexPersistence:
    ...
    async def test_save_and_load_round_trip_should_preserve_unicode_and_emojis(
        ...
        original_index.save(save_dir)
        loaded_index = MemoryIndex.load(save_dir)
    ...
    def test_load_should_raise_file_not_found_error_for_invalid_path(
        self, tmp_path: Path
    ) -> None:
        ...
        with pytest.raises(FileNotFoundError):
            MemoryIndex.load(bad_path)

    @pytest.mark.rag_edge_case
    def test_load_should_fallback_to_pickle_for_legacy_indexes(
        self, tmp_path: Path, memory_index_factory: Callable[..., Any]
    ) -> None:
